################################################################################
# Behavioral simulation of the HLS-generated SoftMax core (N=64, SIMD=1)
# against the precise-fp32 reference embedded in the testbench.
#
...
#   2. Sources the five floating_point IP .tcl files -> creates the fp IP cores
#      the HLS netlist instantiates (fcmp/fsub/fexp/fdiv/fpext). This is the
#      "handle the IP cores accordingly" step: the IP must exist in the project
#      so its simulation model is generated and compiled.
...
# ---- Configuration ----------------------------------------------------------
set top      softmax_wrap_accuracy_tb
set part     xcvc1902-vsva2197-2MP-e-S
set proj     sim_softmax_wrap
...
# plus the SV wrapper (module `softmax`, adapts the HLS core to the reference
# interface). These are design-side so they are visible to both synth and sim.
add_files -norecurse [glob [file join $src_dir *.v]]
add_files -norecurse [file join $src_dir softmax_wrap.sv]

# ---- Simulation-only source: the testbench ----------------------------------
add_files -fileset sim_1 -norecurse [file join $src_dir softmax_wrap_accuracy_tb.sv]

# Treat .sv files as SystemVerilog.
set_property file_type SystemVerilog [get_files [file join $src_dir softmax_wrap.sv]]
set_property file_type SystemVerilog [get_files [file join $src_dir softmax_wrap_accuracy_tb.sv]]
...
# The TB prints:
#   Test (N = 64, SIMD = 1): RMSRE = ..., MAX_REL_ERROR = ..., WORST_INPUT = ...
