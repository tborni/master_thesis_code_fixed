...
`default_nettype none
	...
	"block", "ultra": begin : genMem
		...
		function ptr_t step(input ptr_t  ptr);
			...
			//  1 | 111 |  ...
			//  1 | 111 | 11111
			// NOTE: the "10" prefix lives in the top two bits of ptr, which only
			// exist when ABITS.hi != 0 (then $left(ptr) == ABITS.lo). Address it
			// via the indexed part-select ptr[$left(ptr) -: 2] so the bit indices
			// stay in range even when ABITS.hi == 0 and this branch is dead —
			// a literal ptr[ABITS.lo:ABITS.lo-1] is an out-of-range select in the
			// hi==0 geometry (ptr is only [ABITS.lo-1:0]) and trips Vivado
			// [Synth 8-521] during constant folding before the dead branch is pruned.
			if(ABITS.hi && ptr[$left(ptr) -: 2] == 2'b10)  inc[ABITS.lo-1:ABITS.hi] = '1;
			return  ptr + inc;
		endfunction : step
		...
		if(!IS_URAM) begin : genOutDirect
			...
			uwire  irdy;
			// Non-recursive output queue: a standalone SRL shift FIFO (queue.sv)
			// replaces the former recursive `fifo #(.DEPTH(QDEPTH+1))` instance,
			// which Vivado 2023.1 cannot elaborate (recursive module instantiation
			// -> [Synth 8-521]). queue's ELASTICITY counts shift stages and adds a
			// separate output register, so ELASTICITY=QDEPTH gives capacity
			// QDEPTH+1 -- identical to the original fifo #(.DEPTH(QDEPTH+1)).
			queue #(.DATA_WIDTH(DATA_WIDTH), .ELASTICITY(QDEPTH)) outq (
				.clk, .rst,
