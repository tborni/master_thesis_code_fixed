...
module softmax #(
	...
	// reduction skip the positive branch.  exp_splitting carries its own
	// valid/ready handshake and composes directly with the two diamonds.
	//=======================================================================
	exp_splitting #(
		.SIMD(SIMD),
		.EXCLUDE_POS(1),
	...
	for(genvar  step = 0; step < 2; step++) begin : genDiamonds
		...
		// Reciprocal latency (input valid -> output valid) = SEED 3 + 2*DSP 8 = 11 nominal.
		// The sum is a must-accept, phase-drifting one-per-group operand (see recip below),
		// so the shared-DSP fold runs at SI = NN with a SELF-TIMED accept schedule.  For
		// NN >= 5 the input is a bare wire (genReg) and the schedule is idle when the sum
		// arrives, so latency stays the nominal 11.  For NN in {2,3,4} the fold's 2-deep
		// input skid (genSkid) defers an operand a few cycles to dodge the passA/passB
		// alias (NN | DSP_LATENCY), adding up to 3:  NN in {2,4} -> 14, NN == 3 -> 13.
		// (NN == 1 uses the 2-DSP II=1 config, also 11.)  Guarded further by SLACK and the
		// "Drained bypass" assertion.
		localparam int unsigned  REC_LAT  =
			(NN == 3)?             13 :			// genSkid defer +2
			((NN == 2) || (NN == 4))? 14 :			// genSkid defer +3 (NN | DSP_LATENCY)
			                       11 ;			// NN == 1 (II=1) or NN >= 5 (idle at accept)
		...
		if(1) begin : blkStatistics
			...
			if(step == 0) begin : blkMax
				...
				// rec_bipartite's shared-DSP fold is SELF-TIMED (its accept
