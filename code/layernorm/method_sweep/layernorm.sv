...
module layernorm #(
	...
	//-----------------------------------------------------------------------
	// Inverse-square-root method selection for the variance normalization.
	//
	//	RSQRT_METHOD picks one of the three interchangeable rsqrt cores. They
	//	share the identical streaming handshake (x/xvld/xrdy -> r/rvld) but
	//	differ in the approximation and, therefore, the accuracy/area trade-off:
	//	  "BIPARTITE" - bipartite-table seed + Newton (rsqrt_bipartite)
	//	  "LOOKUP"    - single-table seed + Newton  (rsqrt_lookup)
	//	  "BITHACK"   - Quake magic-constant seed + Newton (rsqrt_bit_hacking)
	//	Only the parameters relevant to the chosen method are consulted; the
	//	rest are ignored (mirrors the per-core parameter sets).
	parameter  RSQRT_METHOD = "BIPARTITE",

	// Newton-Raphson refinement steps applied on top of the seed (0, 1 or 2).
	// 2 steps require an internal II of 1 and are only defined for II = 1;
	// this module forces that automatically (see SUSTAINABLE_INTERVAL below).
	int unsigned  RSQRT_NUM_NEWTON_STEPS = 1,

	// Seed-table mantissa word width, shared by "BIPARTITE" and "LOOKUP".
	int unsigned  RSQRT_WORD_WIDTH = 13,

	// "BIPARTITE"-only table address widths (base / first / second correction).
	int unsigned  RSQRT_ADDR_WIDTH_0 = 2,
	int unsigned  RSQRT_ADDR_WIDTH_1 = 4,
	int unsigned  RSQRT_ADDR_WIDTH_2 = 4,

	// "LOOKUP"-only single-table address width.
	int unsigned  RSQRT_ADDR_WIDTH = 10,

	// "BITHACK"-only magic-constant control. With USE_DEFAULT set, the core
	// selects a Newton-step-tuned constant; otherwise MAGIC_CONSTANT is used.
	bit           RSQRT_USE_DEFAULT_MAGIC_CONSTANT = 1,
	logic [31:0]  RSQRT_MAGIC_CONSTANT = 32'h5F3759DF,

	// Table implementation style forwarded to the table-based cores.
	parameter  RSQRT_RAM_STYLE = "distributed"
)(
	// Global Control
	input	logic  clk,
	input	logic  rst,
	...
	initial begin
		...
		if(!(RSQRT_METHOD == "BIPARTITE" || RSQRT_METHOD == "LOOKUP" || RSQRT_METHOD == "BITHACK")) begin
			$error("%m: RSQRT_METHOD (%s) is invalid. Allowed: BIPARTITE, LOOKUP, BITHACK.", RSQRT_METHOD);
			$finish;
		end
	end
	...
	for(genvar  step = 0; step < 2; step++) begin : genNormDiamonds
		...
		*/
		// Worst-case input->output latency of the selected rsqrt core, used to
		// size the value bypass and statistics queues. It is taken as an upper
		// bound over all methods and Newton-step settings so a single sizing is
		// safe regardless of RSQRT_METHOD/RSQRT_NUM_NEWTON_STEPS: the deepest
		// path is BIPARTITE with 2 Newton steps at II=1 (input reg + decode reg
		// + 2*3*4-stage refinement pipe = 1 + 1 + 24 = 26 cycles). Over-sizing
		// only makes the elastic queues deeper, which is always correct; the
		// bound must never under-count or the bypass would drain / the catcher
		// or rsqrt-input would overrun.
		localparam int unsigned  RSQRT_LATENCY = 26;
		localparam int unsigned  STATISTICS_LATENCY =
			// SIMD adder tree + accumulation + decouple
			$clog2(SIMD) * 2   +     4        +    2 +
			// Variance: square + rsqrt
			step   *    (  3  + RSQRT_LATENCY  );
		localparam int unsigned  VALUE_QUEUE_LEN = NN - 1 + STATISTICS_LATENCY;
		...
		if(1) begin : blkStatistics
			...
			if(1) begin : blkAccumulate
				...
				// Scaled Accumulation
				accuf #(.SCALE(1.0/N), .BIAS(step? EPSILON : 0.0), .FORCE_BEHAVIORAL(FORCE_BEHAVIORAL)) accu (
					.clk, .rst,
			...
			1: /* Var: inverse square root */ begin
				...
				// One new total is produced per input vector, i.e. at most one
				// rsqrt input every NN cycles, so an internal II of NN keeps up.
				// Two Newton steps are only defined at II = 1 (enforced by the
				// cores); force that here, which is throughput-safe since 1 <= NN.
				localparam int unsigned  RSQRT_II = (RSQRT_NUM_NEWTON_STEPS == 2) ? 1 : NN;

				// Instantiate the selected rsqrt core. Only one branch elaborates,
				// so no unused core (or its tables) is built. Each core exposes the
				// identical x/xvld/xrdy -> r/rvld handshake wired below.
				case(RSQRT_METHOD)
				"BIPARTITE": begin : genBipartite
					rsqrt_bipartite #(
						.ADDR_WIDTH_0(RSQRT_ADDR_WIDTH_0),
						.ADDR_WIDTH_1(RSQRT_ADDR_WIDTH_1),
						.ADDR_WIDTH_2(RSQRT_ADDR_WIDTH_2),
						.WORD_WIDTH(RSQRT_WORD_WIDTH),
						.NUM_NEWTON_STEPS(RSQRT_NUM_NEWTON_STEPS),
						.SUSTAINABLE_INTERVAL(RSQRT_II),
						.FORCE_BEHAVIORAL(FORCE_BEHAVIORAL),
						.RAM_STYLE(RSQRT_RAM_STYLE)
					) vari_rsqrt (
						.clk, .rst,
				...
				"LOOKUP": begin : genLookup
					rsqrt_lookup #(
						.ADDR_WIDTH(RSQRT_ADDR_WIDTH),
						.WORD_WIDTH(RSQRT_WORD_WIDTH),
						.NUM_NEWTON_STEPS(RSQRT_NUM_NEWTON_STEPS),
						.SUSTAINABLE_INTERVAL(RSQRT_II),
						.FORCE_BEHAVIORAL(FORCE_BEHAVIORAL),
				...
				"BITHACK": begin : genBithack
					rsqrt_bit_hacking #(
						.NUM_NEWTON_STEPS(RSQRT_NUM_NEWTON_STEPS),
						.SUSTAINABLE_INTERVAL(RSQRT_II),
						.FORCE_BEHAVIORAL(FORCE_BEHAVIORAL),
						.USE_DEFAULT_MAGIC_CONSTANT(RSQRT_USE_DEFAULT_MAGIC_CONSTANT),
						.MAGIC_CONSTANT(RSQRT_MAGIC_CONSTANT)
					) vari_rsqrt (
						.clk, .rst,
						.x(total.dat), .xvld(total.vld), .xrdy(vrdy),
						.r(norm .dat), .rvld(norm .vld)
