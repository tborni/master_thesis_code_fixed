...
module rsqrt_bit_hacking #(
	int unsigned  NUM_NEWTON_STEPS = 1,	// Allowed values: 0, 1 or 2 (only for II = 1)
	...
	bit  USE_DEFAULT_MAGIC_CONSTANT = 1,
	logic [31:0]  MAGIC_CONSTANT = 32'h5F3759DF	// Use specific magic constant, ignored when USE_DEFAULT_MAGIC_CONSTANT is set
)(
	// Global Control
	input	logic  clk,
	input	logic  rst,
	...
	// Choose magic constant based on number of newton steps or use user specified value
	localparam logic [31:0]  MAGIC_ACTUAL =
		USE_DEFAULT_MAGIC_CONSTANT ?
			(NUM_NEWTON_STEPS == 0) ? 32'h5F34C8C3 :
			(NUM_NEWTON_STEPS == 1) ? 32'h5F360742 : 32'h5F3759DF :
		MAGIC_CONSTANT;
	...
	else if(NUM_NEWTON_STEPS == 1) begin
		uwire  xsel;  // Feed new input vs. re-feed for interleaving
		uwire [31:0]  afb;
		uwire [31:0]  a = (xsel? MAGIC_ACTUAL : afb) - (xsel? xx[31:1] : 0);
	...
	else begin : genNewton2	// NUM_NEWTON_STEPS == 2
		uwire [31:0]  afb;
		uwire [31:0]  a = MAGIC_ACTUAL - xx[31:1];
		uwire [31:0]  b = { xx[31], xx[30:23]-1, xx[22:0]}; // 0.5*x
		uwire [31:0]  c = $shortrealtobits(1.5);
