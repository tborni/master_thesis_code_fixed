#!/usr/bin/env python3
"""Parse a Softmax N-sweep accuracy report into a tidy CSV.

The N-sweep varies the Softmax vector length ``N`` while holding the SIMD width
fixed; two reports are produced per sweep, distinguished by the magnitude of the
reference error they were measured against (a ``large_error`` and a
``small_error`` configuration). Every measurement line looks like::

    Test (N = 2, SIMD = 2): RMSRE = 0.0002390009, MAX_REL_ERROR = 0.0006645212, WORST_INPUT = 3.9765934944152832031250000 (elements = 32761)
