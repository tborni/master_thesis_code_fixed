#!/usr/bin/env python3
"""Shared writer for the Softmax N-sweep fit-parameters file.

The resources plot in this experiment fits a model and records its parameters:
``plot_resources.py`` fits an affine ``LUT = m*N + n``. The accuracy figures use
a linear y-axis and are shown as raw sampled points (no fitted model), so they
do not contribute a section here. Rather than the resources script keeping its
own little file, the fit summary lives in a single human-readable file::
...
FILE_HEADER = (
    "# Softmax N-sweep fit parameters\n"
...
def format_affine_line(name: str, var: str, m: float, intercept: float, r_squared: float) -> str:
    ...
    matches the sibling reports so the experiments read alike.
    """
