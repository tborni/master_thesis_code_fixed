...
images/SIMD_<v>/{dsp_only,method_dsp}/ (mirroring the reference scripts
diagram.py / pareto.py / full_pareto.py).  Each set holds the same three
figures:
...
# Faded/vibrant split used by pareto.png / full_pareto.png (from pareto.py).
...
def plot_diagram(rows, enc: Encoding) -> None:
    """Scatter only, no Pareto line (cf. diagram.py)."""
    fig, ax = _new_axes()
...
def plot_pareto(rows, enc: Encoding) -> None:
    """Scatter + single global 2D Pareto staircase (cf. pareto.py)."""
    fig, ax = _new_axes()
...
def plot_full_pareto(rows, enc: Encoding) -> None:
    """Scatter + one Pareto staircase per DSP group (cf. full_pareto.py).
...
def global_limits():
    ...
    ymax = max(ys)
    # log-space margin on x (~5% of a decade each side); 4% headroom on y.
    xpad = (xmax / xmin) ** 0.02
