#!/usr/bin/env python3
"""Shared writer for the Softmax SIMD-sweep fit-parameters file.

The resources plot in this experiment fits a model and records its parameters:
``plot_resources.py`` fits an affine ``LUT = m*SIMD + n``. The accuracy figures
(produced elsewhere) use a linear y-axis and are shown as raw sampled points (no
fitted model), so they do not contribute a section here. Rather than the
resources script keeping its own little file, the fit summary lives in a single
human-readable file::

    data/fit_parameters.txt

organised into ``[section]`` blocks, one per fitting script. Because a script may
run independently (or alone), this module writes each section with an *idempotent
upsert*: :func:`update_section` reads the current file, replaces just the named
section (or appends it if absent), and writes the file back, leaving every other
section and the file header untouched. A fresh run therefore always reflects the
latest fit for that script without clobbering the other's, and repeated runs
converge to the same file.
...
FILE_HEADER = (
    "# Softmax SIMD-sweep fit parameters\n"
...
def _signed(value: float) -> str:
    ...
    ``... + SIMD - 16.7508`` rather than the awkward ``... + SIMD + -16.7508``.
    """
...
def format_affine_line(name: str, var: str, m: float, intercept: float, r_squared: float) -> str:
    ...
    ``var`` is the independent-variable symbol (e.g. ``"SIMD"``). The number
    format matches the sibling reports so the experiments read alike.
    """
...
def format_powerlaw_line(name: str, var: str, r: float, intercept: float, r_squared: float) -> str:
    ...
    fit in log-log space. (Kept for parity with the sibling experiments; the
    Softmax accuracy figures are linear-y and unfitted, so it is presently
    unused here.)
    """
...
def update_section(path: Path, section: str, body_lines: list[str]) -> None:
    ...
    preserved, so multiple plot scripts can each own their section of a single
