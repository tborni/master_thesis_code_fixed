...
method), which is why the generator writes files named after the display label.

A ``filter.toml`` next to this folder optionally restricts which points are
shown (metric bounds, a method allow-list, or per-method Pareto-only).
"""
...
# Okabe-Ito-based palette, one colour per method (matches the sibling sweeps).
...
plt.rcParams.update({
    "font.family": "serif",
    "mathtext.fontset": "stix",   # serif-compatible maths without a LaTeX install
    "font.size": 12,
    "axes.labelsize": 14,
    "axes.titlesize": 14,
    "legend.fontsize": 10,
...
def method_pareto_only(df: pd.DataFrame) -> pd.DataFrame:
    """Keep only the rows of ``df`` on its own (RMSRE, LUT, DSP) Pareto front.
...
def load_combined() -> list[pd.DataFrame]:
    ...
    Files are sorted in reverse so the draw order matches the sibling scripts.
...
def pareto_mask_2d(rmsre: np.ndarray, lut: np.ndarray) -> np.ndarray:
    ...
    LUT, mirroring the staircase drawn on top.
    """
...
def main() -> None:
    ...
    print(f"Rendering figures into {IMAGE_DIR.relative_to(PROJECT_ROOT)}/")
    plot_diagram(frames)
    plot_pareto(frames)
