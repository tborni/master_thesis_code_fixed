#!/usr/bin/env python3
"""Parse a LayerNorm N-sweep accuracy report into a tidy CSV.

The N-sweep varies the LayerNorm vector length ``N`` while holding the SIMD
width fixed; a separate report is produced per invsqrt configuration (0 or 1
Newton refinement steps). Every measurement line looks like::

    Test (N = 2, SIMD = 2): RMSRE = 0.0001728905, MAX_REL_ERROR = 0.0007581115, WORST_INPUT = 1448.2115478515625 (elements = 27360)
...
The trailing ``WORST_INPUT`` / ``elements`` diagnostics describe the worst-case
input and sample count and are not extracted. Lines that do not match the
expected format (blank lines, comments, ...) are skipped and reported on stderr.

There is one report per Newton-step configuration, so the Makefile invokes this
tool once per file with matching ``--input`` / ``--output`` paths (e.g.
``accuracy_0_newton.txt`` -> ``accuracy_0_newton.csv``).
"""
...
DEFAULT_INPUT = DATA_DIR / "accuracy_0_newton.txt"
DEFAULT_OUTPUT = DATA_DIR / "accuracy_0_newton.csv"
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Preserve the report's own ordering: the sweep is already written in
    # ascending N, which is the natural x-axis order for the downstream plots.
    if not records:
